...
module.exports = {
  ...
  before: function (scope, done) {
    ...
    // //
    // // e.g. if someone runs:
    // // $ sails generate migration user find create update
    // // then `scope.args` would be `['user', 'find', 'create', 'update']`
    if (_.isUndefined(scope.args[0])) {
     return done(new Error('Please provide a name for this migration.'));
    }
    ...
    // // Decide the output filename for use in targets below:
     scope.filename = scope.createdAt + '_' + scope.args[0] + '.js';
    //
